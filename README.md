...
| `100099` | `100098` + argument suspect (`.sh`/`.ps1`/`.exe`, `/tmp/`, `/dev/shm/`) | T1105 | hérite de `100098` + `regex` PCRE2 sur la ligne brute |
| `100100` | Exécution de `pwsh`/`powershell` (baseline, niveau 3) | — | hérite de `80792` + `audit.command` |
| `100101` | `100100` + argument suspect (`-enc`, `IEX`, `DownloadString`, bypass) | T1059.001 | hérite de `100100` + `regex` PCRE2 sur la ligne brute |
| `100103` | `100098` répété (≥3 en 90s) **vers la même destination** | T1071 | corrélation par fréquence + `same_field` sur `audit.execve.a1` |
| `100105` | Élévation sudo root précédée d'une connexion SSH (120s) | T1021.004 | corrélation `if_matched_sid` (5715) + `if_sid` (5402), fréquence 2 |
| `100107` | Exécution de `nc`/`ncat` (sondage de port) | T1046 | hérite de `80792` + `audit.command` |

**Révision 1 (audit externe)** : la première version (100099/100101 matchant uniquement le nom du process, 100103/100105 comptant une simple fréquence sans vérifier la destination ou la session) produisait de vrais faux positifs potentiels — un `curl` légitime ou un `pwsh` bénin déclenchait la même alerte de haute criticité qu'une réelle récupération de payload. Corrigée en séparant une règle baseline (niveau 3) d'une règle à criticité élevée nécessitant un indicateur réel dans les arguments.

**Révision 2 (redéploiement et vérification réelle sur la VM, pas seulement des tests unitaires statiques)** — trois bugs supplémentaires, invisibles dans le XML seul, ont été trouvés en déployant réellement `local_rules.xml` sur `single-node-wazuh.manager-1` et en testant chaque règle avec `wazuh-logtest` contre des lignes `auditd`/`sudo`/`sshd` capturées ou fidèlement reconstruites :

1. **`<field name="full_log" type="pcre2">` ne matche jamais pour les événements décodés par `auditd`** dans cette version de Wazuh — la règle charge sans erreur de syntaxe mais ne se déclenche jamais, y compris sur un motif trivial (`/tmp/`) présent en clair dans la ligne. `full_log` n'est simplement pas un champ interrogeable pour ce décodeur. Corrigé en remplaçant par un `<regex type="pcre2">` sans attribut `field` (qui matche la ligne brute pré-décodage) — confirmé fonctionnel sur `100099` et `100101`, y compris l'insensibilité à la casse (`.SH` en majuscules matché correctement).
2. **`100103` était chaîné sur `100099` (fetch avec extension suspecte) au lieu de `100098` (fetch nu)** : un vrai beaconing C2 utilise des chemins comme `/checkin`, `/beacon`, `/poll` (voir `generate_advanced_scenarios.sh`) qui ne contiennent jamais l'extension suspecte de `100099` — la corrélation n'aurait donc jamais rien eu à observer. Corrigé en chaînant sur `100098` : c'est la répétition vers la même destination qui est le signal, indépendamment du contenu de chaque requête individuelle.
3. **`<same_source_ip/>` puis `<same_user/>` sur `100105` ne peuvent pas fonctionner** : `5402` (sudo) est décodé par `0320-sudo_decoders.xml`, qui n'extrait jamais `srcip` (sudo s'exécute dans une session locale déjà ouverte, sans source réseau) — confirmé en lisant le décodeur directement sur la VM. `<same_user/>` a ensuite semblé être la bonne piste (les deux règles semblent porter le compte local `soc`), mais échoue aussi : le décodeur `sshd` de la règle `5715` stocke le compte connecté dans `dstuser`, tandis que le décodeur `sudo` de la règle `5402` stocke ce même compte dans `srcuser` (et `dstuser` devient `root`, la cible de l'élévation) — deux rôles de champ différents que `same_user` ne peut pas faire correspondre. Confirmé empiriquement via `wazuh-logtest` : retirer `<same_user/>` fait fonctionner la corrélation sur une séquence SSH+sudo réelle ; le remettre la fait échouer silencieusement sur la même entrée. Corrigé en abandonnant la corrélation d'identité et en ne gardant que la proximité temporelle (`if_matched_sid` + `if_sid` + fréquence 2, sans champ de corrélation) — **limite assumée** : sur un hôte réellement multi-utilisateurs, deux comptes différents se connectant en SSH puis utilisant `sudo` dans la même fenêtre de 120 s produiraient un faux positif ; sur ce lab mono-compte, la distinction ne change rien en pratique, ce qui explique pourquoi le problème n'était pas visible avant ce test réel.

**Les sept règles ont été testées positif et négatif via `wazuh-logtest`** sur le manager Wazuh réel après chaque correction (pas seulement validées syntaxiquement) : `curl` bénin → seul `100098` ; `curl -o /tmp/x.sh <url>` → `100098` + `100099` ; `pwsh -Command Get-Date` → seul `100100` ; `pwsh -enc <base64>` → `100100` + `100101` ; 3 `curl` vers la même destination → `100103` au 3ᵉ ; 3 `curl` vers 3 destinations différentes → jamais `100103` ; connexion SSH + `sudo` répété → `100105` ; `nc -z -w2 127.0.0.1 80` → `100107`. **Les captures d'écran 28 à 41 de ce document ont été prises avec la toute première version des règles** (avant ces deux révisions) : elles restent une preuve valide que le pipeline Wazuh → Gemma → TheHive → Cortex fonctionne de bout en bout, mais pas de cette version corrigée.

**Nouvelle série de captures avec la version corrigée, en conditions réelles** (agent réel, indexation réelle, pas `wazuh-logtest`) — manifeste complet avec timestamps et commandes exactes dans [`docs/evidence/EVIDENCE.md`](docs/evidence/EVIDENCE.md) :

![Alerte réelle 100099 (fetch suspect vers /tmp) après correction](docs/screenshots/42_wazuh_alert_100099_v2_verified.png)
![Alerte réelle 100105 (sudo après SSH) après correction](docs/screenshots/43_wazuh_alert_100105_verified.png)
![Alerte réelle 100103 (beaconing vers la même destination) après correction](docs/screenshots/44_wazuh_alert_100103_verified.png)
![Alerte réelle 100107 (sondage nc) après correction](docs/screenshots/45_wazuh_alert_100107_verified.png)
![Alerte réelle 100101 (PowerShell encodé) après correction](docs/screenshots/46_wazuh_alert_100101_verified.png)
