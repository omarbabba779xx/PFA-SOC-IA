...
**Note de cohérence sur les règles `100099`/`100101`/`100103`/`100105`** : la version des règles utilisée pour produire les captures ci-dessous matchait uniquement le nom du process (`curl`, `pwsh`) ou une simple fréquence, sans vérifier les arguments réels ni la destination/session. `scripts/local_rules.xml` a depuis été corrigé, **redéployé sur `single-node-wazuh.manager-1` et vérifié réellement via `wazuh-logtest`** (positif et négatif pour les sept règles) — voir README, section "Détection avancée au niveau commande", pour le détail complet des trois bugs supplémentaires trouvés uniquement en testant en conditions réelles (le champ `full_log` ne matche jamais pour les événements `auditd`, `100103` était chaînée sur la mauvaise règle parente, et la corrélation d'identité `same_source_ip`/`same_user` est structurellement impossible entre `sshd` et `sudo` dans ce ruleset). Les captures ci-dessous restent une preuve valide du fonctionnement du pipeline Wazuh → Gemma → TheHive → Cortex, mais pas encore de la version finale des règles — une nouvelle série de captures reste à prendre.
